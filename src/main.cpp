...
int main(int argc, char* argv[]){
    ...
    program_body_toc = PROGRAM_START_THREE_OP_CODE;                                     // Start all programs generated by the compiler with 'GKZF' , i.e. Jump to the starting address, and stop there until the user presses start.
    ...
    for(std::vector<FileContainer>::size_type i = 0; i != input_files.size(); i++){     // Mutate files on at a time - files are ingested in alphabetical order.
        ...
        std::vector<Segment> segs = input_files[i].dissectSegments();                      // Dissect FileCOntainer object into 'segments' - a breakdown of programm structure into Main Program, Functions and Subroutines.
        
        for(int segment_index = 0; segment_index < segs.size(); segment_index++){           // Iterate through each segment of the program.
            Logging::logConditionalMessage(Globals::dump_tokens, " --- New Segment type=(" + getEnumString(segs.at(segment_index).getSegmentType()) + ") ---\n" );

            std::vector<Statement*> stmts = segs.at(segment_index).buildStatements();           // Break each segment of the program down into a series of 'Statement' objects.
