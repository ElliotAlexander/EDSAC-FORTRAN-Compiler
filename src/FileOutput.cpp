...
namespace FileOutput {
    ...
    bool dumpEDASCFile(std::vector<std::string> input){
        ...
        std::string file_name = Globals::output_file_set ? Globals::output_file : EDSAC_DEFAULT_FILE_OUTPUT;     // If the user has set an output file name - use that. Else, use a default
        output_file.open(file_name);
        Logging::logInfoMessage("Writing EDSAC output to " + file_name);
        for(std::vector<std::string>::iterator it = input.begin(); it != input.end(); ++it){                    // For each line
            output_file << (*it) << std::endl;                                                                  // Pipe that line + an end character to the file.
        }                                                                                                       // EoL characters are ignored by the simulator, so this is fine.
        output_file.close();                                                                                    // Close the output stream, tell the user, return true.
    ...
    //
    // @param std::vector<std::shared_ptr<ThreeOpCode> > input -> An input list of the compelte program body, including symbol tables, as Three Op Code (an internal representation)
    ...
    bool dumpThreeOpCodeFile(std::vector<std::shared_ptr<ThreeOpCode> > input){
        ...
        if(!Globals::dump_three_op_code){
            ...
            output_file.open(file_name);                                                                                        // open the output file
            Logging::logInfoMessage("Writing Three Op Code output to " + file_name);
            for(std::vector<std::shared_ptr<ThreeOpCode> >::iterator it = input.begin(); it != input.end(); ++it){              //  Iterate through each line of the input list
                output_file << (*it)->printToString() << std::endl;                                                             // Call utility method printToString to output three op code to string. Add this to the file
            }
            output_file.close();                                                                                                // Close the file, exit and return to calling class.
            Logging::logInfoMessage("Finished writing " + file_name);
            return true;
        }
    }
