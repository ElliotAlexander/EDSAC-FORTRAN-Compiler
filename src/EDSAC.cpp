...
namespace EDSAC {
    ...
    std::vector<std::string> generateEDSAC(std::vector<std::shared_ptr<ThreeOpCode> > input, std::vector<std::string> libraries) {
        ...
        for(auto & it : input){      // Iterate through Program body.
			...
			if (it->getOperation() == THREE_OP_CODE_OPERATIONS::DATA_SET && !address.empty()) {  // If the instruction is a data value, we need to convert the value to a 'pseudo-order'
			...
			} else if (it->getOperation() == THREE_OP_CODE_OPERATIONS::DATA_SET_FLOAT) {
                if (address.empty()) {
                    address = "0";
                }
