 ...
 *  When called, the class registers itself with the symbol table. 
 *  If no value is found in the symbol table, a placeholder is generated. 
...
Variable::Variable(std::string name_in)
{
    ...
    if(x.found){
        ...
        Logging::logConditionalInfoMessage(Globals::output_symbol_table_operations, std::string("Variable " 
            + std::string(name) + 
...
TOC_RETURN_VALUE Variable::generateThreeOPCode(int &starting_address){
     ...
     * Ultimately, the symbol table handles the instructiojn values for data, 
     * and just returns a reference to where the data values 'will' be, once compilation has finished.
     * **/
    if(x.found){
